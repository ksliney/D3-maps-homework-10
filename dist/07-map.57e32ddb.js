...
},{}],"data/coordinates.csv":[function(require,module,exports) {
"use strict";

module.exports = "/coordinates.91588e9b.csv";
},{}],"data/transit-data.csv":[function(require,module,exports) {
"use strict";

module.exports = "/transit-data.8b88b415.csv";
},{}],"07-map.js":[function(require,module,exports) {
"use strict";
...
var lineWidthScale = d3.scaleLinear().domain([0, 100]).range([0, 20]);
Promise.all([d3.json(require('./data/world.topojson')), d3.csv(require('./data/coordinates.csv')), d3.csv(require('./data/transit-data.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  coordinateData.forEach(function (d) {
    var name = d.name;
    var coords = [d.lon, d.lat];
    coordinateStore.set(name, coords);
  });
  var countries = topojson.feature(json, json.objects.countries);
  svg.append('g').selectAll('.country').data(countries.features).enter().append('path').attr('class', 'country').attr('d', path).attr('fill', 'lightgrey');
  svg.selectAll('.city').data(coordinateData).enter().append('circle').attr('class', 'city').attr('r', 3).attr('transform', function (d) {
    return "translate(" + projection([d.lon, d.lat]) + ")";
  });
  svg.selectAll('.transit').data(transitData).enter().append('path').attr('d', function (d) {
    // What is the 'from' city?
    console.log(d.from); // Get the coordinates based on that city's name

    console.log(coordinateStore.get(d.from)); // Pull out our coordinates

    var fromCoords = coordinateStore.get(d.from);
    var toCoords = coordinateStore.get(d.to); // Build a GeoJSON LineString

    var geoLine = {
      type: 'LineString',
      coordinates: [fromCoords, toCoords] // Feed that to our d3.geoPath()

    };
    return path(geoLine);
  }).attr('fill', 'none').attr('stroke', 'red').attr('stroke-width', function (d) {
    return lineWidthScale(d.amount);
  }).attr('opacity', 0.5).attr('stroke-linecap', 'round');
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","./data/world.topojson":"data/world.topojson","./data/coordinates.csv":"data/coordinates.csv","./data/transit-data.csv":"data/transit-data.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","07-map.js"], null)
//# sourceMappingURL=/07-map.57e32ddb.map