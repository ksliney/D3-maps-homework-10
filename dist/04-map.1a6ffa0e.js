...
},{}],"04-map.js":[function(require,module,exports) {
"use strict";
...
var basicLine = d3.line();
d3.json(require('./data/world.topojson')).then(ready).catch(function (err) {
  return console.log('Failed on', err);
});

function ready(json) {
  var world = topojson.feature(json, json.objects.countries);
  console.log('the world is', world);
  svg.selectAll('.country').data(world.features).enter().append('path').attr('fill', 'lightgrey').attr('stroke', 'white').attr('d', path);
  console.log(nyc);
  console.log(projection(nyc));
  svg.append('circle').attr('r', 5).attr('transform', "translate(" + projection(nyc) + ")");
  svg.append('circle').attr('r', 5).attr('transform', "translate(" + projection(london) + ")");
  svg.append('circle').attr('r', 5).attr('transform', "translate(" + projection(tehran) + ")");
  svg.append('circle').attr('r', 5).attr('transform', "translate(" + projection(perth) + ")");
  var coords = [nyc, london];
  var geoLine = {
    type: 'LineString',
    coordinates: coords // svg.append('path')
    //   .attr('stroke', 'red')
    //   .attr('fill', 'none')
    //   .attr('d', path(geoLine))

  };
  svg.append('path').datum(geoLine).attr('stroke', 'red').attr('fill', 'none').attr('d', path);
  svg.append('path').datum([projection(nyc), projection(london)]).attr('stroke', 'green').attr('fill', 'none').attr('d', basicLine);
}
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","04-map.js"], null)
//# sourceMappingURL=/04-map.1a6ffa0e.map